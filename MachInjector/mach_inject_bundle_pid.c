					   ...
					   pid_t		pid )
{
	...
	if( !err ) {
		frameworkBundle = CFBundleGetBundleWithIdentifier(CFSTR("edu.umich.MachInjector"));
	...
	if( !err ) {
		...
		injectionURL = CFBundleCopyResourceURL( frameworkBundle, CFSTR("InjectorStub.bundle"), NULL, NULL );
