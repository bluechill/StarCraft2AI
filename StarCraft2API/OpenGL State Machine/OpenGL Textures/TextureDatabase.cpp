...
namespace OpenGL
{
	static const string texture_db_file = "/Users/bluechill/Developer/OpenGLInjector/StarCraft2API/SC2 Info/SC2DB.xml";
	static const string texture_db_texture_folder = "/Users/bluechill/Developer/OpenGLInjector/StarCraft2API/SC2 Info/SC2Textures/";
	...
	void TextureDatabase::create_texture(GLuint texture)
	{
		...
		if (!has_texture(texture))
		{
			// New texture
			GLint width, height;
			GLenum type = dynamic_cast<SC2_StateMachine*>(Shared)->m_texture_units[dynamic_cast<SC2_StateMachine*>(Shared)->m_active_texture].first;
			...
			if (width != 0 && height != 0)
			{
				...
				opengl_textures.insert(dynamic_cast<SC2_StateMachine*>(Shared)->m_texture_units[dynamic_cast<SC2_StateMachine*>(Shared)->m_active_texture].second);
			}
		}
	}
