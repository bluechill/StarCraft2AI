...
namespace OpenGL
{
	...
	ARBtoOpenCL::arb_instruction* ARBtoOpenCL::convert_ARB_instruction_to_OpenCL(std::string line)
	{
		...
		for (int i = 1;i < output_input.size();++i)
		{
			if (output_input[i] != "=")
			{
				if (output_input[i].find("[") != string::npos && output_input[i].find("..") == string::npos)
				{
		...
		if (ins.type == VECTOR_INSTRUCTION)
		{
			...
			if ((period_location = ins.output_variable.find_last_of(".")) != string::npos)
			{
				...
				if ((swizzle.find("x") != string::npos ||
					...
					swizzle.find_first_not_of("xyzw") == string::npos)
					ins.swizzle = swizzle;
			}
		}
	}
	...
	{		
		ifstream arb_instructions("/Users/bluechill/Developer/OpenGLInjector/StarCraft2API/OpenGL State Machine/OpenGL Programs/ARBInstructions.cl");
		...
		ifstream structures("/Users/bluechill/Developer/OpenGLInjector/StarCraft2API/OpenGL State Machine/OpenGL Programs/Structures.cl");
		...
		std::function<std::string (const std::string, bool)> replaceAllProgram = [this](std::string input, bool vertex_program)
		{
			...
			if (vertex_program)
			{
				...
				{
					size_t found = line.find("result->color", i);
					...
					if (found != string::npos)
					{
						...
						{
							line.insert(found, ".front.primary");
							i = found + 14;
							continue;
						}
						else if (found < line.size() && line[found] == '.')
						{
							...
							string substr = line.substr(start, index-start);
							...
							else if (substr == "primary" || substr == "secondary")
							{
								line.insert(start, "front.");
								i = index + 6;
							}
							else
								i = index;
				...
				replaceAll(line, "result->texcoord", "result_texcoord");
				replaceAll(line, "result_texcoord.", "result_texcoord[0].");
			}
			else
			{
		...
		for (int i = 0;i < program_lines.size();++i)
		{
			...
			else if (line.find("#program ") != string::npos)
			{
				...
				opencl_program.push_back("__kernel void " + name + " ( __global float4* program_env,\n" +
										 "\t\t\t\t\t\t\t\t__global float4* restrict program_local,\n" +
										 "\t\t\t\t\t\t\t\t__global float* restrict vertex_attrib_array,\n" +
										 "\t\t\t\t\t\t\t\t__global int* restrict vertex_indice_array,\n" +
										 "\t\t\t\t\t\t\t\tconst unsigned long indice_offset,\n" +
										 "\t\t\t\t\t\t\t\tconst unsigned long vertex_render_count,\n" +
										 "\t\t\t\t\t\t\t\t__global struct attrib_info_struct* restrict vertex_attrib_info_array,\n" +
										 "\t\t\t\t\t\t\t\t__global struct result_struct* restrict result_array,\n" +
										 "\t\t\t\t\t\t\t\t__global float4* restrict result_texcoord_array,\n" +
										 "\t\t\t\t\t\t\t\tconst unsigned long result_texcoord_max_size,\n" +
										 "\t\t\t\t\t\t\t\tconst unsigned long count)");
				opencl_program.push_back("{");
				opencl_program.push_back("\tunsigned int global_identifier = get_global_id(0);");
				opencl_program.push_back("\tif (global_identifier < count)");
				opencl_program.push_back("\t{");
				
				opencl_program.push_back("");

				ifstream opencl_setup_code("/Users/bluechill/Developer/OpenGLInjector/StarCraft2API/OpenGL State Machine/OpenGL Programs/OpenCLSetupCode.cl");
	...
	void ARBtoOpenCL::output(std::ostream& os)
	{
