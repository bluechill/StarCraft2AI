...
//	syslog(LOG_NOTICE, "mach_inject_bundle: entered in %s, codeOffset: %td, param: %p, paramSize: %lu\n",
...
pthread_entry(
		mach_inject_bundle_stub_param	*param )
{
//	syslog(LOG_NOTICE, "mach_inject_bundle: entered in pthread_entry, param: %p\n", param);
	assert( param );
...
EventLoopTimerEntry(
		...
		mach_inject_bundle_stub_param	*param )
{
//	syslog(LOG_NOTICE, "mach_inject_bundle: entered in EventLoopTimerEntry, inTimer: %p, param: %p\n", inTimer, param);
	assert( inTimer );
	assert( param );
