...
void convert_param_instruction(declaration& param, int& line, vector<string>& program_lines)
{
...
vector<string> print_declaration(declaration* dec)
{
...
vector<string> print_instruction(instruction* ins)
{
...
vector<string> print_arb_instruction(arb_instruction& ins)
{
...
int main(int argc, const char * argv[]) {
	...
	ifstream in_file("/Users/bluechill/Developer/OpenGLInjector/StarCraft2API/StarCraft2API/SC2Info/SC2Shaders/ModelVertexMain-2906163218.arb");
	...
	std::function<std::string (const std::string, bool)> replaceAllProgram = [](std::string input, bool vertex_program)
	{
		string line = input;
		...
		// TODO: implement RGBA swizzling to XYZW swizzling
	...
	for (int i = 0;i < program_lines.size();++i)
	{
		...
		else if (line.find("#program ") != string::npos)
		{
			...
			opencl_program.push_back("__kernel void " + name + " (__global vertex_struct* vertex, __global result_struct* result, __global fragment_struct* fragment, __global program_struct* program, const unsigned int count)");
			opencl_program.push_back("{");
